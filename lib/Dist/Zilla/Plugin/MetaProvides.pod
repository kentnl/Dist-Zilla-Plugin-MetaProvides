...
Dist::Zilla::Plugin::MetaProvides - Generating and Populating 'provides' in
your META.yml
...
Due to how we still havent' worked out how to dermine if the file we read is
processed or not yet to have versions injected into  them, this option is the 
simple way to say that whatever version we find in a file is bunk, and to use
the value reported by dist-zilla instead for all of them.
...
B<[default]> Inherit version from L<Dist::Zilla>
...
B<[default]> Inherit version from L<Dist::Zilla> when one is missing.
